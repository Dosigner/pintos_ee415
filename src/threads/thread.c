...
// minimum wake_up ticks of threads in sleep list
static int64_t global_ticks; 
/* ++++++++++++++++++++++++++++++++++ */


/* ++++ Project 1.5 BSD Scheduler ++++ */
int load_avg;
...
thread_init (void) 
{
  ...
  list_init (&sleep_list);  // sleep list initialize
  //global_ticks = INT64_MAX; // put maximum value of int
  /* +++++++++++++++++++++++++++++++++ */
...
thread_start (void) 
{
  ...
  /* ++++ Project1.5 BSD Scheduler  ++++ */
  load_avg = LOAD_AVG_DEFAULT; //initialize load_avg
  /* ++++++++++++++++++++++++++++++ */
               ...
               thread_func *function, void *aux) 
{
  ...
  /* ++++ Project1.2 Priority Scheduling ++++ */
...
thread_exit (void) 
{
  ...
  if(thread_report_latency)
    printf("Thread <%s> completed in <%d>ticks\n", thread_current()->name, thread_current()->latency);
...
{
  if(thread_mlfqs)
    return; // in mlfqs scheduler we can't change priority
...
/* ++++ Project1.1 Alarm Clock ++++ */
// situation : from timer_sleep, sleep (os_booted_ticks+ticks)
void
thread_sleep (int64_t ticks)
{
  ...
  // Project1.1 Alarm Clock
  current->wakeup_tick = ticks; // save to wakeup time
  if(current != idle_thread){   // not idle
    list_push_back(&sleep_list, &current->elem);
  }
  
  // global tick vs ticks ; which is smaller update
  update_global_ticks(ticks);
  
  thread_block();
  // Change state RUNNING -> BLOCKED
  // schedule();
...
// situation : from timer_interrupt(), call if ticks >= global_ticks
void
thread_awake(int64_t ticks)
{
  global_ticks = INT64_MAX; // refresh update global ticks
  struct list_elem *e;

  // start head.next -> tail.prev
  for(e=list_begin(&sleep_list); e!=list_end(&sleep_list);){
    // From list_elem -> transform "struct thread"
    struct thread *t = list_entry(e, struct thread, elem);

    // thread's wakeup_tick <= more time passes
    if(t->wakeup_tick <= ticks){
      e = list_remove(&t->elem); //delete
      // and return elem->next

      thread_unblock(t);
      // list_push_back (&ready_list, &t->elem);
      // BLOCKED -> THREAD_READY;
    }
    else{
      update_global_ticks(t->wakeup_tick); //update in all sleep_list
      e = list_next(e);
    }
  }
}
...
void thread_test_preemption(void){
  // check ready_list is empty?
  // priority : current vs ready_list_front
  if(!list_empty(&ready_list) &&
     thread_current()->priority <
     list_entry(list_begin(&ready_list),struct thread, elem)->priority)
    thread_yield();
  // push current thread to ready_list,
  // RUNNING -> THREAD_READY
}
...
remove_with_lock(struct lock *lock)
{
  struct list_elem *e;
  struct thread *current = thread_current();
  // donations list, list_remove(thread->donation_elem)
  for (e=list_begin(&current->donations); e!=list_end(&current->donations);){

    struct thread *t = list_entry(e, struct thread, donation_elem);
    
    if (t->wait_on_lock == lock) // lock 
      e=list_remove(&t->donation_elem); // delete thread->donation_elem
    else
      e=list_next(e);
  }
}
...
refresh_priority(void)
{
  ...
  if (!list_empty(&current->donations)) {
    list_sort (&current->donations, &thread_cmp_donate_priority, NULL);
    //printf("sorting...\n");
    struct thread *front = list_entry(list_front(&current->donations), struct thread, donation_elem);

    // front vs current => bigger one update to current->priority
...
/* ++++ Project1.4 BSD Scheduler ++++ */
void
mlfqs_priority(struct thread *t)
{
  if(t!=idle_thread){
    t->priority = fp_to_int(
			    add_mixed(div_mixed(t->recent_cpu, -4), PRI_MAX-t->nice*2));
  }
}
